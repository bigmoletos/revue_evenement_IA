...
- [-] 3. Implémenter la classification et la déduplication
  - [ ] 3.1 Implémenter `detect_event_type(name, description, source)` dans `scraper.py`
  - [ ]* 3.2 Écrire le test property-based pour la classification par type
  - [ ] 3.3 Implémenter `deduplicate(events)` dans `scraper.py`
  - [ ]* 3.4 Écrire le test property-based pour la déduplication
  - [ ] 3.5 Implémenter le tri et le filtrage temporel
  - [ ]* 3.6 Écrire les tests property-based pour le tri et le filtrage

- [ ] 4. Checkpoint — Vérifier les fonctions utilitaires

- [ ] 5. Implémenter les collecteurs par source
  - [ ] 5.1 Implémenter `fetch_eventbrite(query, max_items)` dans `scraper.py`
  - [ ] 5.2 Implémenter `fetch_meetup(max_items)` dans `scraper.py`
  - [ ] 5.3 Implémenter `fetch_luma(max_items)` dans `scraper.py`
  - [ ] 5.4 Implémenter `fetch_conferences()` dans `scraper.py`
  - [ ] 5.5 Implémenter `fetch_corporate_events()` dans `scraper.py`
  - [ ] 5.6 Implémenter `collect_events()` — orchestrateur principal
...
- [ ] 7. Implémenter la génération HTML et l'envoi email
  - [ ] 7.1 Implémenter `build_html(events, pages_url)` dans `mailer.py`
  - [ ]* 7.2 Écrire les tests property-based pour le rendu HTML
  - [ ] 7.3 Implémenter `build_email_html(events, pages_url)` dans `mailer.py`
  - [ ]* 7.4 Écrire les tests property-based pour l'email
  - [ ] 7.5 Implémenter `send_email(events, pages_url)` dans `mailer.py`
  - [ ]* 7.6 Écrire le test property-based pour le parsing multi-destinataires
  - [ ]* 7.7 Écrire les tests unitaires pour mailer.py

- [ ] 8. Implémenter la publication GitHub Pages et les notifications
  - [ ] 8.1 Implémenter `pages_publisher.py`
  - [ ]* 8.2 Écrire les tests unitaires pour pages_publisher.py
  - [ ] 8.3 Implémenter `notifier.py`

- [ ] 9. Implémenter le point d'entrée et le workflow CI/CD
  - [ ] 9.1 Implémenter `run_ci.py`
  - [ ] 9.2 Créer le workflow GitHub Actions `.github/workflows/revue-evenement-ia.yml`
  - [ ] 9.3 Créer le fichier `README.md`

- [ ] 10. Créer les fixtures de test et le fichier conftest.py
  - [ ] 10.1 Créer `tests/conftest.py` avec les fixtures et générateurs Hypothesis partagés
